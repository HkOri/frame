...
// localhost:3000/api/v0/upload/img
router.post('/img', (req, res)=>{
  // 创建实例，用于处理图片数据
    ...
    form.parse(req, (err, fields, files)=>{
        ...
        let img = files.file[0]     // file 这个key是前端UI的ElementUI中Upload组件中设置name属性的值
        // 要使用文件 fs 读取文件在服务器上的缓存数据流
        // 再将其写入到指定的服务器文件夹中
        let fileName = `${Date.now()}-${img.originalFilename}`
        let imgPath = path.resolve(__dirname, `../public/upload/${fileName}`)
